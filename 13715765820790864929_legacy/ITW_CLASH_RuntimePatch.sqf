...
/*
    ...
    - Objective anchors must retain at least six conscious soldiers; degraded remnants are released back to maneuver and refill pressure is raised instead.
*/
...
ITW_CLASH_fnc_GetHomeBaseSpawn = {
    ...
    if (_homeObjective < 0 || {_homeObjective >= count ITW_Objectives}) exitWith {[]};
...
ITW_CLASH_fnc_GetSupportCorridorSpawn = {
    ...
    if (_baseIndex == BASE_INDEX_NONE && {count _attacks > ITW_ATTACK_AIR_E}) then {
...
ITW_CLASH_fnc_ClassifyGroup = {
    ...
    if ((_result#0) || {(_result#1) isNotEqualTo "support-specialist"}) exitWith {_result};
    ...
    if (_supportUnits isEqualTo [] || {count _supportUnits == count _aliveUnits}) exitWith {_result};

    private _objectiveIndex = VAR_GET_OBJ_IDX(_group);
    if (_objectiveIndex < 0) exitWith {[false,"unassigned-objective",[]]};
    if (isNil "ITW_Zones" || {
        isNil "ITW_ZoneIndex" || {isNil "ITW_ObjContestedOwnerIsFriendly"}
    ...
    }) exitWith {[false,"objective-reassigned",[_assignedObjective,_objectiveIndex]]};

    [true,"eligible",[count _aliveUnits,_objectiveIndex,groupOwner _group]]
};
...
ITW_CLASH_fnc_ObserveWriter = {
    ...
    if (isServer && {
        ...
        if (time - _last >= 10) then {
            ...
            ["impasse-garrison-suppressed",[_id,str _group,count units _group]] call ITW_CLASH_fnc_Log;
        };
        true
    };
...
ITW_CLASH_fnc_GetEgressPoint = {
    ...
    private _corridor = [_preferredObjective] call ITW_CLASH_fnc_GetSupportCorridorSpawn;
    if (_corridor isEqualTo []) exitWith {[]};

    [+(_corridor#0),_corridor#1,_corridor#2]
};

ITW_CLASH_fnc_AcknowledgeReconstitution_V6Base = ITW_CLASH_fnc_AcknowledgeReconstitution;
...
ITW_CLASH_fnc_AuditAnchors = {
    if (isServer && {
    ...
    }) then {
        {
            ...
            [_objectiveIndex,"vacant",ITW_CLASH_MinAnchorSoldiers] call ITW_CLASH_fnc_RequestAnchorRefill;
            ["anchor-degraded-demoted",[
                _objectiveIndex,_id,_conscious,ITW_CLASH_MinAnchorSoldiers
            ]] call ITW_CLASH_fnc_Log;
