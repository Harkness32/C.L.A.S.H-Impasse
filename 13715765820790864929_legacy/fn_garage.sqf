//diag_log ["ITW: fn_garage",_this];
//if (is3DEN && {_fnc_scriptName == "skl_fnc_garage"}) exitwith {_this call ITW_Garage3DEN;};
...
switch _mode do {
	...
	case "Open": {
		...
		with missionnamespace do {
            ...
            ITW_GaragePosASL = getPosASL BIS_fnc_garage_center; // SKL MOD
		};
		with uinamespace do {
			_displayMission = [] call (uinamespace getvariable "bis_fnc_displayMission");
			_displayClass = "RscDisplayGarageSKL";
			...
			private _garageDisplay = _displayMission createDisplay _displayClass;
            [_garageDisplay] call (missionNamespace getVariable "ITW_GarageAddCancelBtn"); // SKL MOD
		};
	};
	...
	case "Init": {
		...
		if (isclass (configfile >> "cfgvehicles" >> _classDefault)) then {
			bis_fnc_garage_centerType = _classDefault + ":";
		};
		...
		INITTYPES
		["InitGUI",[_display,"ITW_Garage"]] call ITW_Arsenal;
		["Preload"] call ITW_Garage;
		["ListAdd",[_display]] call ITW_Garage;
		//["ListSelectCurrent",[_display]] call ITW_Garage;
		if (BIS_fnc_garage_centerType == "") then {["buttonRandom",[_display]] call ITW_Garage;};
		["MouseZChanged",[controlnull,0]] call ITW_Arsenal; //--- Reset zoom
		{
...
*/
		...
		"Exit" call ITW_Arsenal;
	};

	///////////////////////////////////////////////////////////////////////////////////////////
	case "Preload": {
        //--- Non-3DEN, load vehicle types sorted by category
        if (isnil "_data" || cheatsenabled) then {
            ...
            {                            
                ...
                // format for each tabData is [p3dModel,[[name,[cfg,cfg,...],hidSelTxtr], [name,[cfg,cfg...],hidSelTxtr], ...], p3dModel,[[name,[cfg,cfg,...],hidSelTxtr], [name,[cfg,cfg...],hidSelTxtr], ...], ...]
                _model = tolower gettext (_x >> "model");
                _modelID = _items find _model; 
                private _animations = (configproperties [_x >> "animationSources","isclass _x",true]) select {gettext (_x >> "displayName") != "" && {getnumber (_x >> "scope") > 1 || !isnumber (_x >> "scope")}} apply {toLowerANSI str _x};
                private _textures   = (configproperties [_x >> "textureSources"  ,"isclass _x",true]) select {gettext (_x >> "displayName") != ""} apply {toLowerANSI str _x};
                private _hidSelTextures = if (_textures isEqualTo []) then {getArray (_x >> "hiddenSelectionsTextures")} else {[]};
                ...
                if (_modelID < 0) then {
                    ...
                    private _nameId = _modelData findIf {(_x#0) isEqualTo _displayName};
                    scopeName "modelComplete";
                    if (_nameId < 0) then {
                        // new name under model
                        _modelData pushBack [_displayName,[_x]];
                    } else {
                        for "_i" from _nameId to count _modelData - 1 do {
                            (_modelData#_i) params ["_name","_configs"];
                            if (_name isEqualTo _displayName) then {
                                private _cfg = _configs#0;
                                private _anims = (configproperties [_cfg >> "animationSources","isclass _x",true]) select {gettext (_x >> "displayName") != "" && {getnumber (_x >> "scope") > 1 || !isnumber (_x >> "scope")}} apply {toLowerANSI str _x};
                                private _txtrs = (configproperties [_cfg >> "textureSources"  ,"isclass _x",true]) select {gettext (_x >> "displayName") != ""} apply {toLowerANSI str _x};
                                private _hidSelTstrs = if (_txtrs isEqualTo []) then {getArray (_cfg >> "hiddenSelectionsTextures")} else {[]};
                                if (_hidSelTstrs isEqualTo _hidSelTextures) then {
                                    if (_anims isEqualTo _animations && {_txtrs isEqualTo _textures && {_hidSelTstrs isEqualTo _hidSelTextures}}) then {
                                            // found match, add class to className list
                                            _configs pushBack _x;
                                            breakTo "modelComplete";
                                    } else {
                                        private _animIntersect = _anims arrayIntersect _animations;
                                        private _txtrIntersect = _txtrs arrayIntersect _textures;
                                        if (_animIntersect isEqualTo _animations && {_txtrIntersect isEqualTo _textures}) then {
                                            // found match, add class to className list
                                            _configs pushBack _x;
                                            breakTo "modelComplete";
                                        } else {
                                            if (_animIntersect isEqualTo _anims && {_txtrIntersect isEqualTo _txtrs}) then {
                                                // found match, update more inclusive anim and txtrs config as the default one
                                                _configs insert [0,[_x]]; // place this as the default veh since it has more anims/textures
                                                breakTo "modelComplete";
                                            };
                                        };
                                    };
                        ...
                        // didn't find a match, add a new model type
                        _modelData pushBack [_displayName,[_x]];
                    };
                };
	...
	case "ListAdd": {
		_display = _this select 0;
		_data = missionnamespace getvariable "skl_fnc_garage_data";
		_center = (missionnamespace getvariable ["BIS_fnc_arsenal_center",player]);
        {
            ...
            for "_i" from 0 to (count _items - 1) step 2 do {
                ...
                private _modelData = _items select (_i + 1);
                {
                    _x params ["_displayName","_configs"];
                    _modelExample = _configs#0;
                    _lbAdd = _ctrlList lbadd _displayName;
                    _ctrlList lbsetpicture [_lbAdd,gettext (_modelExample >> "picture")];
                    _ctrlList lbsetdata [_lbAdd,(_configs apply {configName _x} joinString ":") + ":"];
                    _ctrlList lbsetvalue [_lbAdd,_i*1000 + _forEachIndex]; // capture 2 numbers in one
                    _ctrlList lbsettooltip [_lbAdd,_displayName];
                    if (true) then { //if (_fullVersion) then {
            ...
            for "_i" from 0 to (lbsize _ctrlList - 1) do {
                if (bis_fnc_garage_centerType in (_ctrlList lbdata _i)) exitWith {
...
*/
		{
			...
			if (_active) then {
				...
				if (_idc != IDC_RSCDISPLAYGARAGE_TAB_SUBANIMATION) then { //--- Don't select animation, it would inverse the state
					['SelectItem',[_display,_display displayctrl (IDC_RSCDISPLAYARSENAL_LIST + _idc),_idc]] call ITW_Garage;
				};
			} else {
		...
		['TabSelectRight',[_display,IDC_RSCDISPLAYGARAGE_TAB_SUBCREW]] call ITW_Garage;
	};
	...
	case "SelectItem": {
		...
		private _value = _ctrlList lbvalue _cursel;
        private _subIndex = _value mod 1000;
        _index = floor (_value/1000);
		...
		switch _idc do {
			...
			case IDC_RSCDISPLAYGARAGE_TAB_STATIC: {
				...
				_data = missionnamespace getvariable "skl_fnc_garage_data";
                private _modelDatas = (_data select _idc) select (_index + 1);
				_modelData = _modelDatas select _subIndex;
                _modelData params ["_displayName","_configs"];
				_cfg = _configs select 0;
				_cfgStats = _cfg;
				_class = configname _cfg;
                _centerDir = direction _center;            
				if (_class != _centerType || !alive _center) then {
					...
					_center = _class createvehiclelocal ASLToAGL _centerPos;
                    
                    _center setVectorUp [0,0,1]; // SKL MOD
                    _center setPosASL (missionNamespace getVariable "ITW_GaragePosASL"); // SKL MOD
                    _center allowdamage false; // SKL MOD
					_center setvelocity [0,0,0]; // SKL MOD
                    uiNamespace setVariable ["ITW_PylonVehicle",_center]; // SKL MOD
                    uiNamespace setVariable ["ITW_Pylon_Default_Loadout",getPylonMagazines _center];
					...
					//_center setpos _centerPos; // SKL MOD comment out
					//if ((_center getvariable ["bis_fnc_arsenal_idc",-1]) >= 0) then {_center setpos _centerPos;}; //--- Move vehicle only when previous vehicle was created by Garage // SKL MOD comment out
					...
					bis_fnc_garage_centerType = ((_item splitString ":") #0 ) + ":"; // set to the first cfg of this modeltype
...
*/
		...
		["SetCrewStatus",[_display]] call ITW_Garage;
		["SetAnimationStatus",[_display]] call ITW_Garage;
		["SetTextureStatus",[_display]] call ITW_Garage;
		if (isclass _cfg) then {
			["ShowItemInfo",[_cfg]] call ITW_Arsenal;
			["ShowItemStats",[_cfgStats]] call ITW_Garage;
		};
	};
					...
					if (is3DEN) then {
						...
						_isVehicle = false;
						{
							...
							for "_l" from 0 to (lbsize _ctrlList - 1) do {
								if ((_vehClass + ":") in (_ctrlList lbdata _l)) then {
	...
	case "buttonExport": {
		...
		['showMessage',[_display,localize "STR_a3_RscDisplayArsenal_message_clipboard"]] call ITW_Arsenal;
	};
		...
		if (ctrlenabled _ctrlTemplateName) then {
            ...
            with missionNamespace do {
                [
                    ...
                    getPylonMagazines _center
                ] call ITW_GarageSaveVehicle;
            };
		} else {   
			//--- Load
			...
			if (_categoryIndex >= 0) then {
				...
				if (_nameID >= 0) then {
					...
					_vehTypeC = (_vehData select 0) + ":";
					_ctrlList = _display displayctrl (IDC_RSCDISPLAYARSENAL_LIST + _categoryIndex);
					for "_i" from 0 to (lbsize _ctrlList - 1) do {
						if (_vehTypeC in (_ctrlList lbdata _i)) exitwith {_ctrlList lbsetcursel _i;};
					};
					...
					[_center,_name,_categoryIndex,_display] spawn { //--- Must be spawned, animations cannot be applied immediately
						...
						private _categoryIndex = _this select 2;
						private _display = _this select 3;

						// [_center,[profilenamespace,_name]] call bis_fnc_loadvehicle ==> convert to add pylon saving
                        private _data = profilenamespace getvariable ["bis_fnc_saveVehicle_data",[]];
                        private _nameID = _data find _name;
                        private _success = false;
                        if (_nameID >= 0) then {
                            private _params = +(_data select (_nameID + 1));
                            if (typeof _center == (_params select 0)) then {
                                {_x set [2,true]; _center animate _x; _center animateDoor _x;} foreach (_params select 1);
                                {_center setobjecttexture [_foreachindex,_x];} foreach (_params select 2);
                                [_center,(_params select 3),true,true,true] call bis_fnc_initVehicleCrew;
                                {
                                    private _mag = _x;
                                    if (typeName _x != "STRING") exitWith {};
                                    _center setPylonLoadout [_forEachIndex+1,_mag,true];
                                } forEach (_params select 4);
                                _success = true;
                            } else {
                        ...
                        if (_success) then {
                            if (is3DEN) then {
                                ['SelectItem',[_display,_display displayctrl (IDC_RSCDISPLAYARSENAL_LIST + _categoryIndex),_categoryIndex]] call ITW_Garage;
                            } else {
                                ["SetCrewStatus",[_display]] call ITW_Garage;
                                ["SetAnimationStatus",[_display]] call ITW_Garage;
                                ["SetTextureStatus",[_display]] call ITW_Garage;
	...
	///////////////////////////////////////////////////////////////////////////////////////////
	case "buttonTry": {
        // overriden as 'Reset Pylons'   
        private _veh = missionnamespace getvariable ["BIS_fnc_arsenal_center",player];    // SKL MOD
        uiNamespace setVariable [format ["ITW_Pylon_Loadout_%1",typeOf _veh],uiNamespace getVariable ["ITW_Pylon_Default_Loadout",[]]]; // SKL MOD
        uiNamespace setVariable ["ITW_PylonVehicle",_veh];       // setup next veh          SKL MOD
        missionNamespace setVariable ["ITW_PylonVeh",objNull];   // trigger refresh         SKL MOD
		...
		//	player setpos (_center getPos [135,(sizeof typeof _center) / 2]);
		...
		if (isclass _vehCfg) then {
			...
			_display = uinamespace getvariable ["bis_fnc_arsenal_display",displaynull];
			{
				...
				for "_l" from 0 to (lbsize _ctrlList - 1) do {
					...
					if (_vehClassC in _lbData && !_isVehicle) then {_vehID = _l; _isVehicle = true;};
				};
				if (_isVehicle) exitwith {
