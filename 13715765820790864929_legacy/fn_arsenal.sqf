//if !(_this#0 in ["Mouse","MouseButtonUp","MouseButtonDown","KeyDown","MouseZChanged","draw3D"]) then {diag_log ["ITW: fn_arsenal",_this]};
/*
	Author: 
...
private _fnc_setUnitInsignia = 
{
	...
	private _materialArray = ["insignia", getText (configfile >> "CfgUnitInsignia" >> _insignia >> "material") call {[_this, DEFAULT_MATERIAL] select (_this isEqualTo "")}];
	private _textureArray = ["insignia", getText (configfile >> "CfgUnitInsignia" >> _insignia >> "texture") call {[_this, DEFAULT_TEXTURE] select (_this isEqualTo "")}];
	
	_unit setVariable ["_fnc_setUnitInsignia_class", _insignia];
	_unit setObjectMaterial _materialArray;
	_unit setObjectTexture _textureArray;
};

private _fnc_getUnitInsignia = { _this getVariable ["_fnc_setUnitInsignia_class", ""] };
...
if (_mode == "Open") exitWith // case insensitive
{
	...
	// if there is insignia on uniform, set it
	_center setVariable ["_fnc_setUnitInsignia_class", [_center] call BIS_fnc_getUnitInsignia];
	...
	with missionnamespace do {
		[missionnamespace,"arsenalPreOpen", [_displayMission, _center]] call bis_fnc_callscriptedeventhandler;
	};
...
switch _mode do
{
	...
	case "Init": 
	{
		...
		["skl_fnc_arsenal"] call bis_fnc_startloadingscreen;
		...
		//INITTYPES 
		["InitGUI", [_display, "skl_fnc_arsenal"]] call ITW_Arsenal;
		["Preload"] call ITW_Arsenal;
		["ListAdd", [_display]] call ITW_Arsenal;
		["ListSelectCurrent", [_display]] call ITW_Arsenal;
		...
		["skl_fnc_arsenal"] call bis_fnc_endloadingscreen;
	};
	...
	case "InitGUI": 
	{
		...
		if (is3DEN) then 
		{
			private _isArsenal = _function == "skl_fnc_arsenal";
		...
		_display displayaddeventhandler ["mousebuttondown","with uinamespace do {['MouseButtonDown',_this] call ITW_Arsenal;};"];
		_display displayaddeventhandler ["mousebuttonup","with uinamespace do {['MouseButtonUp',_this] call ITW_Arsenal;};"];
		//_display displayaddeventhandler ["mousezchanged","with uinamespace do {['MouseZChanged',_this] call ITW_Arsenal;};"];
		_display displayaddeventhandler ["keydown","with (uinamespace) do {['KeyDown',_this] call ITW_Arsenal;};"];
		//_display displayaddeventhandler ["mousemoving","with (uinamespace) do {['Loop',_this] call ITW_Arsenal;};"];
		//_display displayaddeventhandler ["mouseholding","with (uinamespace) do {['Loop',_this] call ITW_Arsenal;};"];

		_ctrlMouseArea = _display displayctrl IDC_RSCDISPLAYARSENAL_MOUSEAREA;
		_ctrlMouseArea ctrladdeventhandler ["mousemoving","with uinamespace do {['Mouse',_this] call ITW_Arsenal;};"];
		_ctrlMouseArea ctrladdeventhandler ["mouseholding","with uinamespace do {['Mouse',_this] call ITW_Arsenal;};"];
		_ctrlMouseArea ctrladdeventhandler ["mousebuttonclick","with uinamespace do {['TabDeselect',[ctrlparent (_this select 0),_this select 1]] call ITW_Arsenal;};"];
		_ctrlMouseArea ctrladdeventhandler ["mousezchanged","with uinamespace do {['MouseZChanged',_this] call ITW_Arsenal;};"];
		...
		_ctrlButtonInterface ctrladdeventhandler ["buttonclick","with uinamespace do {['buttonInterface',[ctrlparent (_this select 0)]] call ITW_Arsenal;};"];
		...
		_ctrlButtonSave ctrladdeventhandler ["buttonclick","with uinamespace do {['buttonSave',[ctrlparent (_this select 0)]] call ITW_Arsenal;};"];

		_ctrlButtonLoad = _display displayctrl IDC_RSCDISPLAYARSENAL_CONTROLSBAR_BUTTONLOAD;
		_ctrlButtonLoad ctrladdeventhandler ["buttonclick","with uinamespace do {['buttonLoad',[ctrlparent (_this select 0)]] call ITW_Arsenal;};"];
		...
		_ctrlButtonTry ctrladdeventhandler ["buttonclick","with uinamespace do {['buttonTry',[ctrlparent (_this select 0)]] call ITW_Garage;};"];

		_ctrlArrowLeft = _display displayctrl IDC_RSCDISPLAYARSENAL_ARROWLEFT;
		_ctrlArrowLeft ctrladdeventhandler ["buttonclick","with uinamespace do {['buttonCargo',[ctrlparent (_this select 0),-1]] call ITW_Arsenal;};"];

		_ctrlArrowRight = _display displayctrl IDC_RSCDISPLAYARSENAL_ARROWRIGHT;
		_ctrlArrowRight ctrladdeventhandler ["buttonclick","with uinamespace do {['buttonCargo',[ctrlparent (_this select 0),+1]] call ITW_Arsenal;};"];
		...
		_ctrlTemplateButtonCancel ctrladdeventhandler ["buttonclick","with uinamespace do {['buttonTemplateCancel',[ctrlparent (_this select 0)]] call ITW_Arsenal;};"];

		_ctrlTemplateButtonDelete = _display displayctrl IDC_RSCDISPLAYARSENAL_TEMPLATE_BUTTONDELETE;
		_ctrlTemplateButtonDelete ctrladdeventhandler ["buttonclick","with uinamespace do {['buttonTemplateDelete',[ctrlparent (_this select 0)],true] call ITW_Arsenal;};"];

		_ctrlTemplateValue = _display displayctrl IDC_RSCDISPLAYARSENAL_TEMPLATE_VALUENAME;
		_ctrlTemplateValue ctrladdeventhandler ["lbselchanged","with uinamespace do {['templateSelChanged',[ctrlparent (_this select 0)]] call ITW_Arsenal;};"];
		...
		if !(isnull _ctrlIcon) then 
		{
...

			{
				...
				
				{
					...
					_x ctrladdeventhandler ["mousezchanged","with uinamespace do {['MouseZChanged',_this] call ITW_Arsenal;};"];
				} 
				foreach [_ctrlIcon,_ctrlTab];

				_sort = _sortValues param [_idc,0];
				_ctrlSort = _display displayctrl (IDC_RSCDISPLAYARSENAL_SORT + _idc);
				_ctrlSort ctrladdeventhandler ["lbselchanged",format ["with uinamespace do {['lbSort',[_this,%1]] call ITW_Arsenal;};",_idc]];
				...
				_ctrlList ctrladdeventhandler ["lbdblclick",format ["with uinamespace do {['ShowItem',[ctrlparent (_this select 0),(_this select 0),%1]] spawn ITW_Arsenal;};",_idc]];
		...
		['TabDeselect',[_display,-1]] call ITW_Arsenal;
		...
		_ctrlButtonClose ctrladdeventhandler ["buttonclick","with uinamespace do {['buttonClose',[ctrlparent (_this select 0)]] spawn ITW_Arsenal;}; true"];
		...
		if (_fullVersion && !is3DEN) then 
		{
			if (missionname == "Arsenal") then 
			{
				...
				_ctrlSpace ctrlshow true;
				{
					...
					_ctrl ctrladdeventhandler ["buttonclick","with uinamespace do {['buttonSpace',_this] spawn ITW_Arsenal;}; true"];
		...
		//--- Camera reset
		["Mouse", [controlnull, 0, 0]] call ITW_Arsenal;
		BIS_fnc_arsenal_draw3D = addMissionEventHandler ["draw3D", { with uiNamespace do { ["draw3D"] call ITW_Arsenal } }];
	...
	case "Preload": 
	{
		...
		if (isNil "_data" || isNull _fastLookupTable || _this isEqualTo true) then // or force preload
		{
			...
			// disallow in loadout
			private _blacklist = ["U_VirtualMan_F", "FakeWeapon"];
			// optional mission blacklist
			private _missionBlacklist = getMissionConfigValue ["arsenalRestrictedItems", []];
			if (_missionBlacklist isEqualType [] && { _missionBlacklist isEqualTypeAll "" }) then { _blacklist append _missionBlacklist };
	...
	case "Exit": 
	{
		...
		// sync identity if enabled
		if (_fullVersion) then
		{
			[_center, face _center, speaker _center] call BIS_fnc_setIdentity;
			[_center, _center call _fnc_getUnitInsignia] call BIS_fnc_setUnitInsignia;
			_center setVariable ["_fnc_setUnitInsignia_class", nil];
		};
...

		{
			if (_fullVersion || { !(_foreachindex in [IDC_RSCDISPLAYARSENAL_TAB_FACE, IDC_RSCDISPLAYARSENAL_TAB_VOICE, IDC_RSCDISPLAYARSENAL_TAB_INSIGNIA]) }) then
			{
				...
				if (ctrltype _ctrlList == 102) then {
					...
					_sortValues = uinamespace getvariable ["bis_fnc_arsenal_sort",[]];
					["lbSort",[[_ctrlSort,_sortValues param [_foreachindex,0]],_foreachindex]] call ITW_Arsenal;
				};
			};
		} 
	...
	case "ListSelectCurrent": 
	{
		...
		
		{
			...
			{
				if (_defaultShow < 0) then { ["ShowItem", [_display, _ctrlList, _foreachindex]] spawn ITW_Arsenal };
		...
		if (_defaultShow >= 0) then {["ShowItem", [_display,_display displayctrl (IDC_RSCDISPLAYARSENAL_LIST + _defaultShow),_defaultShow]] spawn ITW_Arsenal;};
	...
	case "TabDeselect": {
		...
		if ({count _x > 0} count BIS_fnc_arsenal_buttons == 0) then {
			...
			if (!_shown || _key == 1) exitwith {['buttonInterface',[_display]] call ITW_Arsenal;};
	...
	case "TabSelectLeft": {
...

		{
			...
			if (_active) then {
				...
				['SelectItem',[_display,_display displayctrl (IDC_RSCDISPLAYARSENAL_LIST + _idc),_idc]] call ITW_Arsenal;
			};
		...
		if (_showItems) then {['TabSelectRight',[_display,IDC_RSCDISPLAYARSENAL_TAB_ITEMOPTIC]] call ITW_Arsenal;};
		...
		_ctrl ctrlsetfade _fadeCargo;
		_ctrl ctrlcommit FADE_DELAY;
		if (_showCargo) then {['TabSelectRight',[_display,IDC_RSCDISPLAYARSENAL_TAB_CARGOMAG]] call ITW_Arsenal;};
		...
		//['SelectItem',[_display,_display displayctrl (IDC_RSCDISPLAYARSENAL_LIST + _index),_index]] call ITW_Arsenal;
	};
		...
		_ctrBackgroundRight = _display displayctrl IDC_RSCDISPLAYARSENAL_BACKGROUNDRIGHT;
		{
			...
			if (_active) then {
				...
				if (
					...
					["SelectItemRight",[_display,_ctrlList,_index]] call ITW_Arsenal;
				};
			};
	...
	case "SelectItem": 
	{
		...
		private _fnc_addWeapon = 
		{
			...
			[_unit,_weapon] call compile (getText (_weaponCfg >> "VirtualArsenal" >> "Eventhandlers" >> "itemSelected"));
		};
		...
		private _fnc_removeWeapon = 
		{
			...
			_unit removeWeapon [_weapon, false];
			private _magazines = [_weapon] call bis_fnc_compatibleMagazines;
			{_unit removeMagazines _x}foreach _magazines;

			[_unit,_weapon] call compile (getText (_weaponCfg >> "VirtualArsenal" >> "Eventhandlers" >> "itemDeselected"));
		};

		private _fnc_addItem = {
			params ["_unit", "_item","_cfg"];

			private _itemConfig = configFile >> _cfg >> _item;
			[_unit,_item] call compile (getText (_itemConfig >> "VirtualArsenal" >> "Eventhandlers" >> "itemSelected"));
		};
		...
		private _fnc_removeItem = {
			...
			private _itemConfig = configFile >> _cfg >> _item;
			[_unit,_item] call compile (getText (_itemConfig >> "VirtualArsenal" >> "Eventhandlers" >> "itemDeselected"));
		};
		
		private _fnc_addItemVerify = {
			params ["_unit", "_item","_slot"];

			private _itemPrevious = "";
			private _cfg = "";
			switch(_slot)do
			{
				case "headger" : 	{_itemPrevious = headgear _unit;_cfg = "CfgWeapons"};
				case "goggles" : 	{_itemPrevious = goggles _unit;_cfg = "CfgGlasses"};
				case "vest" : 		{_itemPrevious = vest _unit;_cfg = "CfgWeapons"};
				case "hmd" : 		{_itemPrevious = hmd _unit;_cfg = "CfgWeapons"};
				case "uniform" : 	{_itemPrevious = uniform _unit;_cfg = "CfgWeapons"};
				case "backpack" : 	{_itemPrevious = backpack _unit;_cfg = "CfgVehicles"};
			};
			if(_itemPrevious isNotEqualTo _item)then{[_unit,_itemPrevious,_cfg] call _fnc_removeItem};

			[_unit,_item,_cfg] call _fnc_addItem;
		};
		...
		switch _index do 
		{
			case IDC_RSCDISPLAYARSENAL_TAB_UNIFORM: 
			{
				...
				//["SelectItem",[_display, _display displayctrl (IDC_RSCDISPLAYARSENAL_LIST + IDC_RSCDISPLAYARSENAL_TAB_INSIGNIA),IDC_RSCDISPLAYARSENAL_TAB_INSIGNIA]] spawn ITW_Arsenal;
				[_center, _center call _fnc_getUnitInsignia] call _fnc_setUnitInsignia;
				
			};
			case IDC_RSCDISPLAYARSENAL_TAB_VEST: 
			{
				[_center,_item,"vest"] call _fnc_addItemVerify;
				if (_item == "") then {
					removevest _center;
				} else {
			...
			case IDC_RSCDISPLAYARSENAL_TAB_BACKPACK: {
				[_center,_item,"backpack"] call _fnc_addItemVerify;
			...
			case IDC_RSCDISPLAYARSENAL_TAB_HEADGEAR: {
				[_center,_item,"headgear"] call _fnc_addItemVerify;
				if (_item == "") then {removeheadgear _center;} else {_center addheadgear _item;};
			};
			case IDC_RSCDISPLAYARSENAL_TAB_GOGGLES: {
				[_center,_item,"goggles"] call _fnc_addItemVerify;
			...
			case IDC_RSCDISPLAYARSENAL_TAB_PRIMARYWEAPON: 
			{
				...
				if (_weapon call BIS_fnc_baseWeapon != _item) then 
				{
					...
					{if(!(_x in _newWeaponItems))then{[_center,_x,"CfgWeapons"] call _fnc_removeItem}}forEach _oldWeaponItems;
			...
			case IDC_RSCDISPLAYARSENAL_TAB_INSIGNIA: 
			{
				[_center, _item] call _fnc_setUnitInsignia;
			};
				...
				switch true do {
					case (ctrlenabled _ctrlListPrimaryWeapon): {
						if (_item != "") then {
							...
							if (_weaponAccessories select _accIndex != _item) then {
								[_center,_weaponAccessories select _accIndex,"CfgWeapons"] call _fnc_removeItem;
							};
							_center addprimaryweaponitem _item;
							[_center,_item,"CfgWeapons"] call _fnc_addItem;
						} else {
							_weaponAccessories = _center weaponaccessories primaryweapon _center;
					...
					case (ctrlenabled _ctrlListSecondaryWeapon): {
						if (_item != "") then {
							...
							_weaponAccessories = _center weaponaccessories secondaryweapon _center;
							if (count _weaponAccessories > 0) then {
								_center removesecondaryweaponitem (_weaponAccessories select _accIndex);
								[_center,_weaponAccessories select _accIndex,"CfgWeapons"] call _fnc_removeItem;
							};
						};
					};
					case (ctrlenabled _ctrlListHandgun): {
						if (_item != "") then {
							...
							_weaponAccessories = _center weaponaccessories handgunweapon _center;
							if (count _weaponAccessories > 0) then {
								_center removehandgunitem (_weaponAccessories select _accIndex);
								[_center,_weaponAccessories select _accIndex,"CfgWeapons"] call _fnc_removeItem;
							};
						};
					};
		...
		if (
			...
			_weaponAccessories = _center weaponaccessories _weapon;
			{
				...
				["lbSort",[[_ctrlSort,lbcursel _ctrlSort],_x]] call ITW_Arsenal;
			} foreach [
				IDC_RSCDISPLAYARSENAL_TAB_ITEMMUZZLE,
				IDC_RSCDISPLAYARSENAL_TAB_ITEMACC,
				IDC_RSCDISPLAYARSENAL_TAB_ITEMOPTIC,
		...
		if (ctrlenabled _ctrlList) then 
		{
			...
			if (BIS_fnc_arsenal_type == 0 || (BIS_fnc_arsenal_type == 1 && !is3DEN)) then 
			{
				["ShowItemInfo",[_itemCfg]] call ITW_Arsenal;
				["ShowItemStats",[_itemCfg]] call ITW_Arsenal;
			};
		};
	};
		...
		if (isclass _itemCfg) then {
			...
			if (_dlc != ""/* && _fullVersion*/) then {
				...
				if (_appId > 0) then {
					...
					{ _ctrlDLC ctrlRemoveAllEventHandlers _x }foreach ["mouseexit","mouseenter","buttonclick"];
				};
			} else {
		...
		switch true do {
			case (_key == DIK_ESCAPE): {
				if (_inTemplate) then {
					...
					if (_fullVersion) then {["buttonClose",[_display]] spawn ITW_Arsenal;} else {_display closedisplay 2;};
				};
				_return = true;
			};
			...
			case (_key in [DIK_RETURN,DIK_NUMPADENTER]): {
				...
				if (ctrlfade _ctrlTemplate == 0) then {
					if (BIS_fnc_arsenal_type == 0) then {
						["buttonTemplateOK",[_display]] spawn ITW_Arsenal;
					} else {
						["buttonTemplateOK",[_display]] spawn ITW_Garage;
					};
					_return = true;
				};
			};
			...
			case (_key == DIK_TAB): {
				...
				if (BIS_fnc_arsenal_type == 0) then {
					["TabSelectLeft",[_display,_idc]] call ITW_Arsenal;
				} else {
					["TabSelectLeft",[_display,_idc]] call ITW_Garage;
				};
				_return = true;
			};

			//--- Export to script (Ctrl+C), export to config (Ctrl+Shift+C)
			case (_key == DIK_C): { if (_ctrl) then { ['buttonExport', [_display, ["init", "config"] select _shift]] call ([ITW_Garage, bis_fnc_arsenal] select (BIS_fnc_arsenal_type == 0)) } };
			
			//--- Import from script (Ctrl+V)
			case (_key == DIK_V): { if (_ctrl) then { ['buttonImport' ,[_display]] call ([ITW_Garage, bis_fnc_arsenal] select (BIS_fnc_arsenal_type == 0)) } };
			
			//--- Save (Ctrl+S)
			case (_key == DIK_S): { if (_ctrl) then { ['buttonSave', [_display]] call ITW_Arsenal } };
			
			//--- Open (Ctrl+O)
			case (_key == DIK_O): { if (_ctrl) then {['buttonLoad',[_display]] call ITW_Arsenal } };
				...
				if (_ctrl) then {
					if (BIS_fnc_arsenal_type == 0) then {
						if (_shift) then {
							...
							['buttonRandom',[_display]] call ITW_Arsenal;
						};
					} else {
						['buttonRandom',[_display]] call ITW_Garage;
					};
				};
			};
			//--- Toggle interface
			case (_key == DIK_BACKSPACE && !_inTemplate): {
				['buttonInterface',[_display]] call ITW_Arsenal;
				_return = true;
			};
...
/*
			...
			case (_key == DIK_DELETE): {
				...
				if !(ctrlenabled _ctrlMouseBlock) then {
					['buttonTemplateDelete',[_display]] call ITW_Arsenal;
					_return = true;
				};
			};
*/
		...
		else 
		{
			...
			if ((_ctrlTemplateValue lnbValue [_cursel, 0]) >= 0 || _ctrlTemplateValue lnbData [_cursel, 0] == "override") then 
			{
				...
				if (_nameID >= 0 && !is3DEN) then
				{
					...
					[_center, _insignia] call _fnc_setUnitInsignia;
				};
	...
	case "buttonTemplateDelete": 
	{
		...
		if (_confirmAction isEqualTo true) exitWith
		{
			_this spawn 
			{
				...
				{ 
					isNil { with uinamespace do { ["buttonTemplateDelete", _this, !_confirmAction] call ITW_Arsenal } };
				};
			};
		};
		...
		['showTemplates',[_display]] call (uinamespace getvariable (["skl_fnc_arsenal","skl_fnc_garage"] select BIS_fnc_arsenal_type));
		_ctrlTemplateValue lnbsetcurselrow (_cursel max (lbsize _ctrlTemplateValue - 1));

		["templateSelChanged",[_display]] call ITW_Arsenal;
/*
...
*/
	...
	case "showTemplates": 
	{
		...
		for "_i" from 0 to (count _data - 1) step 2 do 
		{
			...
			{
				private _allgear = [];
				_allgear pushback (_primary select 0);
				_allgear append (_primary select 1);
				_allgear pushback (_primary select 2);				
				_allgear pushback (_secondary select 0);
				_allgear append (_secondary select 1);
				_allgear pushback (_secondary select 2);
				_allgear pushback (_handgun select 0);
				_allgear append (_handgun select 1);
		...
		//["templateSelChanged",[_display]] call ITW_Arsenal
		//['buttonExport',[_display]] call ITW_Arsenal;
	};
	...
	case "buttonImport": 
	{
		...
		else 
		{
			...
			_to = 1;
			{
				...
				switch tolower _x do 
				{
					...
					case "adduniform": 
					{
						if (CONDITION(_virtualItemCargo)) then {[_center,_item,"uniform"] call _fnc_addItemVerify;_center forceadduniform _item} else {ERROR};
					};
					case "addvest": 
					{
						if (CONDITION(_virtualItemCargo)) then {[_center,_item,"vest"] call _fnc_addItemVerify;_center addvest _item} else {ERROR};
					};
					case "addbackpack":
					{
						if (CONDITION(_virtualBackpackCargo)) then {[_center,_item,"backpack"] call _fnc_addItemVerify;_center addbackpack _item} else {ERROR};
					};
					case "addheadgear": 
					{
						if (CONDITION(_virtualItemCargo)) then {[_center,_item,"headgear"] call _fnc_addItemVerify;_center addheadgear _item} else {ERROR};
					};
					case "addgoggles": 
					{
						if (CONDITION(_virtualItemCargo)) then {[_center,_item,"backpack"] call _fnc_addItemVerify;_center addgoggles _item} else {ERROR};
					};
					...
					case "bis_fnc_setidentity":
					{
						if (_fullVersion) then 
						{ 
							private _face = _importArray param [_foreachindex - 5, ""]; 
							if (_face isNotEqualTo "") then { _center setFace _face };
							private _speaker = _importArray param [_foreachindex - 3, ""]; 
							if (_speaker isNotEqualTo "") then { _center setSpeaker _speaker };
						};
					};
					case "bis_fnc_setunitinsignia": 
					{
						if (_fullVersion) then 
						{ 
							private _insignia = _importArray param [_foreachindex - 3, ""]; 
							[_center, _insignia] call _fnc_setUnitInsignia;
						};
					};
				};
		...
		{
			["showMessage", [_display, localize "STR_A3_RscDisplayArsenal_message_unavailable"]] call ITW_Arsenal;
		};
	...
	case "buttonExport":
	{
		private _exportCenter = missionnamespace getvariable ["BIS_fnc_arsenal_center", player];
		private _exportInsignia = _exportCenter call _fnc_getUnitInsignia;
		[_exportCenter, _this select 1, _fullVersion, _exportInsignia] call BIS_fnc_exportInventory spawn { copytoclipboard _this };
		["showMessage", [_this select 0, localize "STR_a3_RscDisplayArsenal_message_clipboard"]] call ITW_Arsenal;
	};
...
*/
	...
	case "buttonSave": 
	{
		...
		['showTemplates',[_display]] call (uinamespace getvariable (["skl_fnc_arsenal","skl_fnc_garage"] select BIS_fnc_arsenal_type));//bis_fnc_arsenal;
		...
		["showMessage", [_display,localize "STR_A3_RscDisplayArsenal_message_save"]] call ITW_Arsenal;
	};
	...
	case "buttonSpace": {
		...
		_function = ["skl_fnc_arsenal","skl_fnc_garage"] select _buttonID;
	...
	case "AmmoboxInit": 
	{
		...
		[["AmmoboxServer",_box,true],"skl_fnc_arsenal",false] call bis_fnc_mp;
	};
	...
	case "AmmoboxExit": 
	{
		...
		[["AmmoboxServer",_box,false],"skl_fnc_arsenal",false] call bis_fnc_mp;
	};
	...
	case "AmmoboxServer": 
	{
		...
		["AmmoboxLocal","skl_fnc_arsenal",true,isnil "bis_fnc_arsenal_ammoboxServer"] call bis_fnc_mp;
			...
			if (isnil {_x getvariable "bis_fnc_arsenal_action"}) then {
				_action = _x addaction [
					localize "STR_A3_Arsenal",
					{
						...
						["Open",[nil,_box,_unit]] call ITW_Arsenal;
					},
					[],
					6,
					true,
