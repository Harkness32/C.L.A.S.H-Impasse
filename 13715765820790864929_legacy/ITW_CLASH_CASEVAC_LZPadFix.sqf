...
// Apply only after the CASEVAC air-ops correction owns the inbound route.
waitUntil {
    sleep 0.1;
...
// Give the helicopter a persistent landing object while keeping the infantry
// out from underneath the rotor disk. The rally point is placed approximately
// 30 m from the pad on the side from which the squad is approaching.
ITW_CLASH_CASEVAC_fnc_OrderLZ = {
    ...
    if (isNull _pad) exitWith {
        // Fail open to the existing coordinate behavior rather than cancelling
        // an otherwise valid extraction because the helper object could not spawn.
        [_group,_lz] call ITW_CLASH_CASEVAC_fnc_OrderLZ_V1Base
    };
    _pad setPosATL _lz;
...
ITW_CLASH_CASEVAC_fnc_OrderHeliLZ = {
    ...
    // landAt is local to the aircraft. CASEVAC crews are intentionally kept
    // server-local, so this server-side command owns the precision landing.
    ...
    // Keep reasserting the helipad while the extraction remains inbound. This
    // prevents the generic landing FSM from drifting to a nearby alternate LZ.
